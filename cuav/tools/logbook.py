...
def add_log_entry(logfile, takeoff_time, flight_time, distance):
...
def flight_time(logfile):
    ...
    while True:
        m = mlog.recv_match(type=['GPS','GPS_RAW_INT','VFR_HUD'], condition=args.condition)
        if m is None:
            if in_air:
                total_time += time.mktime(t) - start_time
            if total_time > 0:
                print("Flight time %s %u:%02u" % (logfile, int(total_time)/60, int(total_time)%60))
            if in_air and total_dist >= args.mindist*1000 and total_time >= args.mintime*60:
