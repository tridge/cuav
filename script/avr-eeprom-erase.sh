...
echo "Erasing EEPROM on device $DEV"
...
dd if=/dev/zero of=4kzero.bin bs=1024 count=4

avrdude -C$AVR_CONF -i 10 -p$DEVICE -c$STK -P"$DEV" -b$BAUD -D $* -Ueeprom:w:4kzero.bin:r
