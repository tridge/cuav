...
/*
  load 1280x960 8 bit images as saved by the chameleon capture code
  image should be 1280x960
  This discards all of the header meta-data for fast operation
 */
static bool pgm_load_chameleon(const char *filename, struct grey_image8 *image)
{
	int fd;
	char hdr[128];
	const char *p;
	...
	p += 5;
	if (pread(fd, &image->data[0][0], WIDTH*HEIGHT, p-hdr) != WIDTH*HEIGHT) {
		close(fd);
		return false;
	}
	close(fd);
 ...
 */
static void colour_convert_chameleon(const struct grey_image8 *in, struct rgb_image8 *out)
{
	unsigned x, y;
	/*
