...
#define AVERAGE_LOW    70
#define AVERAGE_HIGH   85
#define AVERAGE_TARGET 78

#define SATURATED_HIGH 100
#define SATURATION_THRESHOLD 234
...
static void get_averages(uint8_t *image, float *average, 
...
static void adjust_shutter(float *shutter, float average, uint32_t num_saturated, uint32_t num_half_saturated)
{
...
static void capture_wait(struct chameleon_camera *c, float *shutter, const char *basename, bool testonly, 
			 struct timeval *tv)
{
	struct chameleon_frame *frame;
	uint8_t buf[IMAGE_HEIGHT*IMAGE_WIDTH];
	float average;
	...
	printf("%s shutter=%f average=%.1f saturated=%u hsaturated=%u\n", 
	       fname, *shutter, average, num_saturated, num_half_saturated);
	...
	if (!testonly && fork() == 0) {
		...
		dprintf(fd,"P5\n%u %u\n#PARAM: t=%llu shutter=%f average=%.1f saturated=%u\n255\n", 
			IMAGE_WIDTH, IMAGE_HEIGHT, (unsigned long long)timestamp, *shutter, 
	...
	adjust_shutter(shutter, average, num_saturated, num_half_saturated);
...
static void capture_loop(struct chameleon_camera *c1, struct chameleon_camera *c2, float framerate, const char *basename, bool testonly)
{
	...
	while (true) {
		...
		if (c1) {
			capture_wait(c1, &shutter[0], basenames[0], testonly, &tv);
		}
		if (c2) {
			capture_wait(c2, &shutter[1], basenames[1], testonly, &tv);
		}
	}
}
